...
def _open_findings(deps: Any) -> list[str]:
    ...
    if conf:
        L += [f"### 7.1 断言求值为假的发现 ({len(conf)} 条)", "",
              "每一条都附带一个针对产物的断言, 而该断言在本次交付的产物上**求值为假**。"
              "**求值为假的是断言, 不等于结论成立。**", "",
              "> ⚠️ **这一节要按「待查」读, 不要按「缺陷清单」读。** 在 live40 上对全部 13 "
              "条机器证实的发现做独立复核, 只有 **2 条**是真实缺陷; 8 条的算术完全正确而"
              "**结论是错的**。最主要的一种错法是: 两个字段本来就在度量**不同的总体**"
              "(不同的样本、不同的口径、不同的 id 空间), 于是「两处对不上」既真实又无害。", "",
