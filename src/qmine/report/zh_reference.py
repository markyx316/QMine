...
_WHAT_FOR: dict[str, tuple[str, str, str]] = {
    ...
    "report_pooled":   ("pooled/跨快照对比_逐类对照.zh.md", "要比较快照时读",
                        "每一个意图、每一个聚类叶在每一个快照上的占比、区间与缺席可检出性; "
                        "以及每个快照的特征类与逐类真实例子。"),
    "workbook_pooled": ("pooled/跨快照对比_对照表.xlsx", "要自己算跨快照的数时打开",
                        "上面那份报告底下的全部表格, 每层一页。"),
    "workbook_pooled_rows": ("pooled/跨快照对比_逐行标注.xlsx", "要按行查某个快照时打开",
                        "每一行查询, 它来自哪个快照, 以及两条路线各自给它的标签。"),
