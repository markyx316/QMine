...
for d in CAT:
    ...
    # controls
    h=1000; b,_=nn(ES[:h],ES[h:]); ctrl+=[dict(domain=d,kind="对照A",query=q,sim=float(v)) for q,v in zip(qs[:h],b)]; print(f"  对照A 搜索top1000 -> 搜索1001+      median {np.median(b):.3f} | "+bands(b))
    t=len(ES)-1000; b,_=nn(ES[t:],ES[:t]); ctrl+=[dict(domain=d,kind="对照B",query=q,sim=float(v)) for q,v in zip(qs[t:],b)]; print(f"  对照B 搜索最深1000 -> 搜索其余     median {np.median(b):.3f} | "+bands(b))
...
pd.DataFrame(rows).to_parquet(f"{SP}/sva_semantic_nn_v3.parquet",index=False); pd.DataFrame(ctrl).to_parquet(f"{SP}/sva_semantic_nn_controls_v3.parquet",index=False)
