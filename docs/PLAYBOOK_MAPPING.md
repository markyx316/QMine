...
| 3 | Metrics must not betray the objective | [`records.py::METRIC_AUTHORITY`](../src/qmine/records.py), [`panel.py`](../src/qmine/ops/panel.py) | `decisive_ranking()` **raises** on an advisory metric — silhouette cannot select anything |
| 4 | Stability over any single internal metric | [`cluster.py::replay_stability`](../src/qmine/ops/cluster.py) | every selection key is `stability_ari` first; silhouette breaks ties only |
